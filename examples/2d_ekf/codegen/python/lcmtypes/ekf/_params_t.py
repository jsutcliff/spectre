# This file automatically generated by skymarshal
# DO NOT MODIFY BY HAND
# fmt: off
# isort: off
# mypy: disallow-untyped-defs

import copy
import typing as T  # pylint: disable=unused-import

from io import BytesIO
import struct

class params_t(object):
    __slots__: T.List[str] = ["m"]

    def __init__(
        self,
        m: float=0.0,
        _skip_initialize: bool=False,
    ) -> None:
        """ If _skip_initialize is True, all other constructor arguments are ignored """
        if _skip_initialize:
            return
        self.m: float = m

    @staticmethod
    def from_all_fields(
        m: float,
    ) -> "params_t":
        return params_t(
            m=m,
        )

    @staticmethod
    def _skytype_meta() -> T.Dict[str, str]:
        return dict(
            type="struct",
            package="ekf",
            name="params_t",
        )

    @classmethod
    def _default(cls) -> "params_t":
        return cls()

    def __repr__(self) -> str:
        return "params_t({})".format(
            ", ".join("{}={}".format(name, repr(getattr(self, name))) for name in self.__slots__))

    def __eq__(self, other: object) -> bool:
        if not isinstance(other, params_t):
            return NotImplemented
        return (
            (self.m==other.m)
        )
    # Disallow hashing for python struct lcmtypes.
    __hash__ = None  # type: ignore

    def encode(self) -> bytes:
        buf = BytesIO()
        buf.write(params_t._get_packed_fingerprint())
        self._encode_one(buf)
        return buf.getvalue()

    def _encode_one(self, buf: T.BinaryIO) -> None:
        buf.write(params_t._CACHED_STRUCT_0.pack(self.m))

    @staticmethod
    def decode(data: T.Union[bytes, T.BinaryIO]) -> "params_t":
        # NOTE(eric): This function can technically accept either a BinaryIO or
        # anything that supports the C++ Buffer Protocol,
        # which is unspecifiable in type hints.

        if hasattr(data, "read"):
            # NOTE(eric): mypy isn't able to figure out the hasattr check
            buf = T.cast(T.BinaryIO, data)
        else:
            buf = BytesIO(T.cast(bytes, data))

        if buf.read(8) != params_t._get_packed_fingerprint():
            raise ValueError("Decode error")
        return params_t._decode_one(buf)

    @staticmethod
    def _decode_one(buf: T.BinaryIO) -> "params_t":
        self = params_t(_skip_initialize=True)
        self.m = params_t._CACHED_STRUCT_0.unpack(buf.read(8))[0]
        return self

    @staticmethod
    def _get_hash_recursive(parents: T.List[T.Type]) -> int:
        if params_t in parents: return 0
        tmphash = (0x66493de0f5c67da) & 0xffffffffffffffff
        tmphash = (((tmphash<<1)&0xffffffffffffffff)  + (tmphash>>63)) & 0xffffffffffffffff
        return tmphash

    _packed_fingerprint: T.Optional[bytes] = None

    @staticmethod
    def _get_packed_fingerprint() -> bytes:
        if params_t._packed_fingerprint is None:
            params_t._packed_fingerprint = struct.pack(">Q", params_t._get_hash_recursive([]))
        return params_t._packed_fingerprint

    def deepcopy(self, **kwargs: T.Any) -> "params_t":
        """
        Deep copy of this LCM type

        Returns a copy w/ members specified by kwargs replaced with new values specified by kwargs.
        """
        result = copy.deepcopy(self)
        for key in kwargs:
            if not hasattr(result, key):
                raise KeyError("Type params_t does not have attribute: " + str(key))
            setattr(result, key, kwargs[key])
        return result

    _CACHED_STRUCT_0 = struct.Struct(">d")
